(function () {
  ...
  function open(plan) {
    ...
    status.textContent = 'We check compatibility before setup. Paystack securely handles the monthly billing.';
    dialog.showModal();
  }
  ...
  form.addEventListener('submit', function (event) {
    ...
    var endpoint = '/website-purchases';
    fetch(API + endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      ...
      .then(function (body) { if (body.paymentUrl) { window.location.assign(body.paymentUrl); } else throw new Error('Paystack did not return a payment page.'); })
