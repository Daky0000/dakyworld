...
- Browser checks use the real editor through `client/builder-harness.html?editor`. Start Vite on port 5199, install Playwright in a local development environment, then run `node checks/browser/builderRedesign.mjs`. `PLAYWRIGHT_URL` can point to an existing Playwright module. The harness is excluded from the production Vite entry.
