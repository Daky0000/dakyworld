...
export function CssValueField({ property, label, name, prefix, value, disabled, bare, onChange }: {
  ...
  const box = `h-7 w-full rounded-lg border bg-sunken ${prefix ? "pl-[18px] pr-1.5" : "px-2"} font-mono text-[11px] text-ink outline-none transition placeholder:text-faint focus:border-blue focus:ring-2 focus:ring-blue/25 hover:border-line-strong ${error ? "border-danger-solid" : "border-transparent"}`;
  ...
  const problem = error ? <span role="alert" className="mt-1 block text-[11px] text-danger-text">Use a valid CSS value (px, %, rem or auto).</span> : null;
  // Bare: the row it sits in already says what the property is called.
  if (bare) return <span className="relative block min-w-0 flex-1">{prefix && <span aria-hidden className="pointer-events-none absolute left-1.5 top-1/2 -translate-y-1/2 font-sans text-[11px] uppercase text-faint">{prefix}</span>}{field}{problem}</span>;
