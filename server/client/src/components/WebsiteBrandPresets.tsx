...
export function WebsitePresetPicker({ presets, kind, tag, style, onApply }: { presets: BrandPreset[]; kind: FieldKind; tag: string; style: string; onApply: (style: string) => void }) {
  ...
  return <div className="mb-3"><label className="block text-xs font-semibold">Brand style<select aria-label="Brand style" className="mt-1 w-full rounded-lg border border-line p-2 text-sm" value="" disabled={!available.length} onChange={e => { const preset = available.find(item => item.id === e.target.value); if (preset) onApply(mergePreset(style, preset)); }}><option value="">{available.length ? "Choose an approved style" : "No approved styles for this element"}</option>{available.map(preset => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></label></div>;
}
