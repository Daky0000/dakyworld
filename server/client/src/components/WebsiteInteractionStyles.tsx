  ...
  return <details open className="border-b border-line px-3 py-3"><summary className="cursor-pointer text-sm font-semibold">Hover, focus & active</summary><p className="my-2 text-xs text-muted">Interaction styles apply at every screen size. Focus styles appear during keyboard navigation.</p>
    ...
    <fieldset disabled={readOnly} className="space-y-2">{INTERACTION_PROPERTIES.map(property => (
      property.includes("color") ? (
        <div className="flex items-center justify-between gap-2 text-xs capitalize" key={`${state}:${property}`}>
        ...
        <label className="block text-xs capitalize" key={`${state}:${property}`}>{property.replaceAll("-", " ")}<input key={`${state}:${property}:${declarations[`--dw-${state}-${property}`] ?? ""}`} aria-label={`${state} ${property}`} defaultValue={declarations[`--dw-${state}-${property}`] ?? ""} placeholder={property === "opacity" ? "0.8" : property === "transform" ? "translateY(-2px)" : property === "text-decoration" ? "underline" : "0 4px 12px #00000033"} className="mt-1 w-full rounded-[10px] border border-line bg-white p-2 text-xs" onBlur={event => { if (event.target.value !== (declarations[`--dw-${state}-${property}`] ?? "")) set(property, event.target.value); }} /></label>
      )
    ))}
