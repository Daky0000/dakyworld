...
export function WebsiteImageFraming({ src, siteId, style, onApply }: { src: string; siteId?: string; style: string; onApply: (style: string) => void }) {
  ...
  return <details className="mb-3 rounded-xl border border-line p-3">
    ...
    <label className="block text-xs">Shape<select aria-label="Shape" className="my-2 block w-full rounded-[10px] border border-line p-2" value={ratio} onChange={e => setRatio(e.target.value)}>{!IMAGE_ASPECTS.includes(ratio as typeof IMAGE_ASPECTS[number]) && <option value={ratio}>Current ratio ? {ratio}</option>}<option value="16 / 9">Wide · 16:9</option><option value="4 / 3">Landscape · 4:3</option><option value="1 / 1">Square</option><option value="3 / 4">Portrait · 3:4</option></select></label>
    <div className="grid grid-cols-2 items-start gap-2">{["Desktop", "Phone"].map(device => <figure key={device} className={device === "Phone" ? "mx-auto w-3/4" : "w-full"}><figcaption className="mb-1 text-xs text-muted">{device} framing</figcaption><button type="button" aria-label={`Set focal point in ${device} image`} className="relative block w-full overflow-hidden rounded-[10px] bg-sunken" style={{ aspectRatio: ratio }} onClick={e => { const rect = e.currentTarget.getBoundingClientRect(); setX(clampFocal(Math.round(100 * (e.clientX - rect.left) / rect.width))); setY(clampFocal(Math.round(100 * (e.clientY - rect.top) / rect.height))); }}><img src={previewSrc} alt="Image crop preview" onLoad={e => setDimensions({ width: e.currentTarget.naturalWidth, height: e.currentTarget.naturalHeight })} onError={() => setFailed(true)} className="h-full w-full" style={{ objectFit: "cover", objectPosition: position }} /><span aria-hidden className="pointer-events-none absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-blue" style={{ left: `${x}%`, top: `${y}%` }} /></button></figure>)}</div>
    ...
    <button type="button" disabled={failed} className="rounded-[10px] bg-ink px-3 py-2 text-xs text-white disabled:opacity-50" onClick={() => onApply(writeStyle({ ...existing, "object-fit": "cover", "object-position": position, "aspect-ratio": ratio, width: "100%", height: "auto" }))}>Apply crop to draft</button>
  </details>;
}
