...
export function WebsiteImageFraming({ src, siteId, style, onApply }: { src: string; siteId?: string; style: string; onApply: (style: string) => void }) {
  ...
  return <details className="mb-3 rounded-xl border border-line p-3">
    ...
    <label className="block text-xs">Shape<select aria-label="Shape" className="my-2 block w-full rounded-lg border border-line p-2" value={ratio} onChange={e => setRatio(e.target.value)}><option value="16 / 9">Wide · 16:9</option><option value="4 / 3">Landscape · 4:3</option><option value="1 / 1">Square</option><option value="3 / 4">Portrait · 3:4</option></select></label>
    <div className="grid grid-cols-2 items-start gap-2">{["Desktop", "Phone"].map(device => <figure key={device} className={device === "Phone" ? "mx-auto w-3/4" : "w-full"}><figcaption className="mb-1 text-xs text-muted">{device} framing</figcaption><button type="button" aria-label={`Set focal point in ${device} image`} className="relative block w-full overflow-hidden rounded-lg bg-sunken" style={{ aspectRatio: ratio }} onClick={e => { const rect = e.currentTarget.getBoundingClientRect(); setX(Math.round(100 * (e.clientX - rect.left) / rect.width)); setY(Math.round(100 * (e.clientY - rect.top) / rect.height)); }}><img src={previewSrc} alt="Image crop preview" onError={() => setFailed(true)} className="h-full w-full" style={{ objectFit: "cover", objectPosition: position }} /><span aria-hidden className="pointer-events-none absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-blue" style={{ left: `${x}%`, top: `${y}%` }} /></button></figure>)}</div>
    ...
    {([["Horizontal", x, setX], ["Vertical", y, setY]] as const).map(([label, value, update]) => <label key={label} className="mt-3 block text-xs">{label} focal point · {value}%<input className="block w-full" type="range" min="0" max="100" value={value} onChange={e => update(Number(e.target.value))} /></label>)}
