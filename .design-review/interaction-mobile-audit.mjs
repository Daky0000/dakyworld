...
const browser=await chromium.launch();const page=await browser.newPage({viewport:{width:390,height:844}});const errors=[];const results=[];page.on('pageerror',e=>errors.push(e.message));
...
writeFileSync('.design-review/live/interaction-mobile-audit.json',JSON.stringify({results,errors},null,2));console.log(JSON.stringify({results,errors},null,2));await browser.close();
